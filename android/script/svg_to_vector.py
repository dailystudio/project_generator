...
from svg_leaf_node import SvgLeafNode
...
class Svg2Vector:
    ...
    def parse(self, f: str) -> SvgTree:
        ...
        self.traverse_svg_and_extract(svg_tree, root, root_element)

        # Fill in all the <use> nodes in the svgTree.
        nodes = svg_tree.get_pending_use_set()
        while nodes:
            if not any(self.extract_use_node(svg_tree, node, node.get_document_element()) for node in list(nodes)):
                # Not able to make progress because of cyclic references.
                self.report_cycles(svg_tree, nodes)
                break
        ...
        for class_name, affected_nodes in svg_tree.get_style_affected_nodes():
            for n in affected_nodes:
                self.add_style_to_path(n, svg_tree.get_style_class_attr(class_name))
        ...
        for entry in svg_tree.get_clip_path_affected_nodes_set():
            ...
            self.handle_clip_path(svg_tree, node, group_node, value)
    ...
    def report_cycles(self, svg_tree: SvgTree, svg_nodes: Set[SvgGroupNode]):
        edges = {}
        nodes_by_id = {}
        for svg_node in svg_nodes:
            ...
            if id_attr:
                target_id = element.getAttribute(self.SVG_HREF)
                if not target_id:
                    target_id = element.getAttribute(self.SVG_XLINK_HREF)
                if target_id:
                    edges[id_attr] = self.get_id_from_reference(target_id)
        ...
        while edges:
            ...
            if target_id:  # Broken links are reported separately. Ignore them here.
                ...
                cycle = self.get_cycle_starting_at(id_node, edges, nodes_by_id)
    ...
    def traverse_svg_and_extract(self, svg_tree: SvgTree, current_group: SvgGroupNode, item: Node):
        ...
        for i in range(child_nodes.length):
            ...
            if tag_name in (self.SVG_PATH, self.SVG_RECT, self.SVG_CIRCLE, self.SVG_ELLIPSE, self.SVG_POLYGON, self.SVG_POLYLINE, self.SVG_LINE):
                child = SvgLeafNode(svg_tree, child_element, tag_name + str(i))
                self.process_id_name(svg_tree, child)
                current_group.add_child(child)
                self.extract_all_items_as(svg_tree, child, child_element, current_group)
                svg_tree.set_has_leaf_node(True)

            elif tag_name == self.SVG_GROUP:
                child_group = SvgGroupNode(svg_tree, child_element, "child" + str(i))
                current_group.add_child(child_group)
                self.process_id_name(svg_tree, child_group)
                self.extract_group_node(svg_tree, child_group, current_group)
                self.traverse_svg_and_extract(svg_tree, child_group, child_element)

            elif tag_name == self.SVG_USE:
                child_group = SvgGroupNode(svg_tree, child_element, "child" + str(i))
                self.process_id_name(svg_tree, child_group)
                current_group.add_child(child_group)
                svg_tree.add_to_pending_use_set(child_group)

            elif tag_name == self.SVG_DEFS:
                child_group = SvgGroupNode(svg_tree, child_element, "child" + str(i))
                self.traverse_svg_and_extract(svg_tree, child_group, child_element)

            elif tag_name in (self.SVG_CLIP_PATH_ELEMENT, self.SVG_MASK):
                clip_path = SvgClipPathNode(svg_tree, child_element, tag_name + str(i))
                self.process_id_name(svg_tree, clip_path)
                self.traverse_svg_and_extract(svg_tree, clip_path, child_element)

            elif tag_name == self.SVG_STYLE:
                self.extract_style_node(svg_tree, child_element)

            elif tag_name == "linearGradient":
                gradient_node = SvgGradientNode(svg_tree, child_element, tag_name + str(i))
                self.process_id_name(svg_tree, gradient_node)
                self.extract_gradient_node(svg_tree, gradient_node)
                ...
                if tag_name in self.unsupported_svg_nodes:
                ...
                self.traverse_svg_and_extract(svg_tree, current_group, child_element)


    def extract_gradient_node(self, svg: SvgTree, gradient_node: SvgGradientNode):
        ...
        for i in range(gradient_children.length):
            ...
            if node_name == "stop":
                ...
                for k in range(stop_attr.length):
                    ...
                    try:
                        if name == "offset":
                            ...
                            greatest_offset = self.extract_offset(value, greatest_offset)
                ...
                offset = self.format_float_value(greatest_offset)
    ...
    def extract_group_node(self, svg_tree: SvgTree, child_group: SvgGroupNode, current_group: SvgGroupNode):
    ...
    def process_id_name(self, svg_tree: SvgTree, node: SvgGroupNode):
        id_attr = node.get_attribute_value("id")
        if id_attr:
            svg_tree.add_id_to_map(id_attr, node)

    def extract_use_node(self, svg_tree: SvgTree, use_group_node: SvgGroupNode, current_node: Node) -> bool:
        ...
        for j in range(attributes.length):
            ...
            name = n.nodeName
            value = n.nodeValue
            if name == self.SVG_HREF:
                id_value = self.get_id_from_reference(value)
            elif name == self.SVG_XLINK_HREF and id_value is None:
                id_value = self.get_id_from_reference(value)
            elif name == "x":
                x = float(value)
            elif name == "y":
                y = float(value)
            elif name in self.presentation_map:
    ...
    @staticmethod
    def get_id_from_reference(self, value: str) -> str:
        return value[1:] if value else ""

    def handle_clip_path(self, svg: SvgTree, child: SvgNode, current_group: Optional[SvgGroupNode], value: Optional[str]):
        if current_group is None or value is None:
            return

        clip_name = self.get_clip_path_name(value)
    ...
    def extract_all_items_as(self, svg: SvgTree, child: SvgLeafNode, current_item: Node, current_group: SvgGroupNode):
        ...
        while parent_node and parent_node.nodeName == "g":
            ...
            self.println_common(parent_node)

            attr = parent_node.attributes
            node_attr = attr.getNamedItem(self.SVG_STYLE)
            ...
            display_attr = attr.getNamedItem(self.SVG_DISPLAY)
        ...
        self.println_common(current_item)

        if has_node_attr and style_content:
            self.add_style_to_path(child, style_content)

        if current_item.nodeName == self.SVG_PATH:
            self.extract_path_item(svg, child, current_item, current_group)
        elif current_item.nodeName == self.SVG_RECT:
            self.extract_rect_item(svg, child, current_item, current_group)
        elif current_item.nodeName == self.SVG_CIRCLE:
            self.extract_circle_item(svg, child, current_item, current_group)
        elif current_item.nodeName in (self.SVG_POLYGON, self.SVG_POLYLINE):
            self.extract_poly_item(svg, child, current_item, current_group)
        elif current_item.nodeName == self.SVG_LINE:
            self.extract_line_item(svg, child, current_item, current_group)
        elif current_item.nodeName == self.SVG_ELLIPSE:
            self.extract_ellipse_item(svg, child, current_item, current_group)

        # Add the type of node as a style class name for child.
        svg.add_affected_node_to_style_class(current_item.nodeName, child)

    @staticmethod
    def println_common(self, n: Node):
    ...
    def extract_poly_item(self, svg_tree: SvgTree, child: SvgLeafNode, current_group_node: Node,
        ...
        if current_group_node.nodeType == Node.ELEMENT_NODE:
            ...
            for item_index in range(attributes.length):
                ...
                value = n.nodeValue
                try:
                    if name == self.SVG_STYLE:
                        self.add_style_to_path(child, value)
                    elif name in self.presentation_map:
                        ...
                        builder = PathBuilder()
                        split = self.SPACE_OR_COMMA.split(value)
                        ...
                        for j in range(2, len(split), 2):
                            ...
                            base_x = x
                            base_y = y
                        if current_group_node.nodeName == self.SVG_POLYGON:
    ...
    def extract_rect_item(self, svg: SvgTree, child: SvgLeafNode, current_group_node: Node, current_group: SvgGroupNode):
    ...
    def extract_circle_item(self, svg: SvgTree, child: SvgLeafNode, current_group_node: Node,
                            current_group: SvgGroupNode):
    ...
    def extract_ellipse_item(self, svg: SvgTree, child: SvgLeafNode, current_group_node: Node,
                             current_group: SvgGroupNode):
    ...
    def extract_line_item(self, svg: SvgTree, child: SvgLeafNode, current_group_node: Node,
                          current_group: SvgGroupNode):
    ...
    def extract_path_item(self, svg: SvgTree, child: SvgLeafNode, current_group_node: Node,
                          current_group: SvgGroupNode):
    ...
    def add_style_to_path(self, path: SvgNode, value: Optional[str]):
        ...
        if value:
            ...
            for k in range(len(parts) - 1, -1, -1):
                ...
                if len(name_value) == 2 and name_value[0] and name_value[1]:
                    ...
                    if attr in self.presentation_map:
                        ...
                        path.fill_presentation_attributes(self.SVG_FILL_OPACITY, name_value[1])
    ...
    def parse_svg_to_xml(self, input_svg: str, out_stream) -> str:
        """
        ...
        """
        svg_tree = self.parse(input_svg)
        if svg_tree.get_has_leaf_node():
            self.write_file(out_stream, svg_tree)
